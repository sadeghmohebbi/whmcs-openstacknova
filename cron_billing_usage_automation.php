...
$hourlyRatePerGB = 0.10; // $0.10 per GB it should load from product configuration
...
$clients = localAPI($command, $postData, $adminUsername);

foreach ($clients['clients']['client'] as $client) {
    $clientId = $client['id'];
    $clientName = $client['firstname'] . ' ' . $client['lastname'];
    echo "Client ID: {$clientId}, Name: {$clientName}\n";

    // Fetch active services/products for each client via Local API

    $command = 'GetClientsProducts';
    $postData = array(
        'stats' => true,          // Optional: includes basic stats
        'clientid' => $clientId,        // Optional: specify client ID to filter
    );
    ...
    echo "Total Charge for Client ID {$clientId}: \${$totalCharge}\n";
